...
- ✅ **동영상 파일 검색**: Everything 검색 엔진 통합으로 드라이브 전체에서 동영상 파일을 초고속 검색 ✨
- ✅ **탭 기반 UI**: 인코딩과 검색 기능을 탭으로 분리하여 직관적인 워크플로우 제공 ✨
...
### 검색 탭 ✨
1. **드라이브 선택**: 검색할 드라이브 선택 (C:\, D:\ 등)
2. **필터 설정**: 
   - 컨테이너: 특정 확장자만 검색 (mp4, mkv 등) 또는 전체
   - 최소 크기: 파일 크기 필터 (100MB, 1GB 등)
3. **검색 시작**: "🔍 검색 시작" 버튼 클릭
   - Everything 설치 시: 초고속 검색 (NTFS 인덱싱)
   - Everything 미설치 시: 표준 OS 검색 (느림)
4. **결과 확인**: 검색 결과 테이블에서 파일명, 경로, 크기, 확장자 확인
5. **인코딩으로 전송**: 원하는 파일 선택 후 "➡️ 선택한 파일을 인코딩 탭으로 보내기" 클릭
